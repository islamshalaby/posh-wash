...
class UserController extends AdminController{
    ...
    public function EditPost(Request $request){
        $check_user_phone = User::where('phone' , $request->phone)->where('id' , '!=' , $request->id)->where('deleted_at', '!=', null)->first();
        if($check_user_phone){
            return redirect('admin-panel/users/edit/'.$request->id)->with('status', 'Phone Exists Before');
        }

        $check_user_mail = User::where('email' , $request->email)->where('id' , '!=' , $request->id)->where('deleted_at', '!=', null)->first();
        if($check_user_mail){
    ...
    public function AddPost(Request $request){
        $check_user_phone = User::where('phone' , $request->phone)->where('deleted_at', '!=', null)->first();
        if($check_user_phone){
            return redirect('admin-panel/users/add')->with('status', 'Phone Exists Before');
        }

        $check_user_mail = User::where('email' , $request->email)->where('deleted_at', '!=', null)->first();
        if($check_user_mail){
